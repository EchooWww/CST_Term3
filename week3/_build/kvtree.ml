...
(** [largest] returns the value associated with the largest key in [t]. *)
let rec largest t =
  match t with 
...
(** [smallest] returns the value associated with the smallest key in [t]. *)
let rec smallest t =
  match t with
...
let rec delete k t =
  ...
  | Node (_, l, Leaf) -> l
  | Node ((xkey,xval), l, r)->
    if (largest l) > (smallest r) 
    then Node((largest l, xval), delete (largest l) l, r)
    else Node((smallest r, xval), l, delete (smallest r) r)
